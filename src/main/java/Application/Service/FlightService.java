...
public class FlightService {
    ...
    public Flight updateFlight(int flight_id, Flight flight){
        // Check if the flight with the given flight_id exists
        Flight existingFlight = flightDAO.getFlightById(flight_id);
        if (existingFlight == null) {
            return null; // return null if flight doesn't exist
        }

        // Update the existing flight with the new data
        flightDAO.updateFlight(flight_id, flight);

        // Retrieve the updated flight from the database and return it
